...
import { generateEmbedding, processTextForEmbedding, createEntityEmbedding } from "../../utils/aiUtils.js";
...
class AiController {
  ...
  async generateEmbeddings(req, res, next) {
    try {
      ...
      if (!validTypes.includes(entity_type)) {
        throw new CustomError(`Invalid entity type. Must be one of: ${validTypes.join(', ')}`, 400);
      }
  ...
  async getDealCoach(req, res, next) {
    try {
      ...
      const dealData = await this.aiRepository.getDealDataForAnalysis(deal_id, company_id);
      ...
      if (!dealData || dealData.activities.length === 0) {
        return res.status(200).json(new Response(
          ...
          {
            cold_start: true,
            message: "Not enough data for AI analysis. Add more activities to get insights.",
            recommendations: [],
            health_score: null,
            risks: []
          }
      ...
      const relevantContext = await this.aiRepository.getRelevantContextForDeal(deal, company_id);
      ...
      try {
        ...
        dealCoachAnalysis = await this.generateDealCoachAnalysisWithAI(dealData);
      } catch (aiError) {
        ...
        dealCoachAnalysis.ai_error = "AI service unavailable, showing rule-based analysis";
      }
  ...
  async getPersonaBuilder(req, res, next) {
    try {
      ...
      const contactData = await this.aiRepository.getContactDataForAnalysis(contact_id, company_id);
      ...
      // Get relevant context using vector similarity search
      const relevantContext = await this.aiRepository.getRelevantContextForContact(contact, company_id);
      ...
      try {
        ...
        personaAnalysis.ai_error = "AI service unavailable, showing rule-based analysis";
      }
  ...
  async handleObjection(req, res, next) {
    try {
      ...
      const objectionEmbedding = await this.aiRepository.generateEmbeddingForText(objection_text);
      ...
      if (deal_id) {
        ...
        dealData = await this.aiRepository.getDealDataForAnalysis(deal_id, company_id);
      }

      // Find similar objections using vector search for context
      const relevantContext = await this.aiRepository.getRelevantContextForObjection(
        objectionEmbedding,
        company_id
      );
      ...
      try {
        // Try to use the AI-powered analysis
        objectionAnalysis = await this.generateObjectionAnalysisWithAI(objection_text, dealData, relevantContext);
      } catch (aiError) {
        console.error("AI analysis failed, falling back to rule-based analysis:", aiError);
        // Fallback to rule-based analysis if AI fails
        objectionAnalysis = this.generateObjectionAnalysis(objection_text, dealData);
        objectionAnalysis.ai_error = "AI service unavailable, showing rule-based analysis";
      }
  ...
  async getWinLossExplainer(req, res, next) {
    try {
      ...
      if (deal.status !== 'won' && deal.status !== 'lost') {
        throw new CustomError("Deal must be closed (won or lost) for win-loss analysis", 400);
      }
      ...
      try {
        ...
        winLossAnalysis.ai_error = "AI service unavailable, showing rule-based analysis";
      }
  ...
  generateDealCoachAnalysis(dealData) {
    ...
    const hasRecentActivity = activityCount > 0 && 
      (new Date() - new Date(activities[0].created_at)) / (1000 * 60 * 60 * 24) < 7;
    ...
    if (!hasRecentActivity) {
      recommendations.push("Schedule a follow-up call this week to re-engage the prospect");
    }
    
    if (daysInStage > 20) {
      recommendations.push("Deal has been in current stage for too long. Consider discussing timeline or addressing potential blockers");
    }
    
    if (activityCount < 3) {
      recommendations.push("Increase engagement with more touchpoints (emails, calls, meetings)");
    }
  ...
  async generateDealCoachAnalysisWithAI(dealData) {
    try {
      ...
      const activitySummaries = activities.map(activity => {
        return `- ${new Date(activity.created_at).toLocaleDateString()}: ${activity.type} - ${activity.content || 'No content'}`;
      }).join('\n');
      
      // Format relevant context
      const contextSummaries = relevantContext ? relevantContext.map(ctx => {
        return `- ${ctx.type}: ${ctx.summary}`;
  ...
  generatePersonaAnalysis(contactData) {
    ...
    const communicationStyles = ["Direct", "Analytical", "Consensus-driven", "Expressive"];
    const motivators = ["Cost savings", "Efficiency", "Innovation", "Reliability", "Status"];
    const decisionPatterns = ["Data-driven", "Emotional", "Collaborative", "Authority-based"];
    ...
    const decisionPattern = decisionPatterns[Math.floor(Math.random() * decisionPatterns.length)];
    
    return {
      cold_start: false,
      persona: {
        communication_style: style,
        description: `${contact.name} tends to communicate in a ${style.toLowerCase()} manner, focusing on ${selectedMotivators[0].toLowerCase()} and ${selectedMotivators[1].toLowerCase()}.`
      },
      ...
      engagement_tips: [
        ...
        `Provide ${decisionPattern === "Data-driven" ? "detailed analytics" : "clear outcomes"}`,
        `Follow up with ${style === "Direct" ? "concise summaries" : "comprehensive details"}`
      ]
    };
  }
  ...
  async generatePersonaAnalysisWithAI(contactData) {
    try {
      ...
      const dealSummaries = deals && deals.length > 0 ? deals.map(deal => {
  ...
  generateObjectionAnalysis(objectionText, dealData) {
    ...
    return {
      ...
      tone_advice: category === "Price" ? "Empathetic but value-focused" : 
                  category === "Timing" ? "Patient but highlighting opportunity cost" :
                  "Confident and consultative"
    };
  }
  
  // AI-powered method to generate Objection analysis using Gemini
  async generateObjectionAnalysisWithAI(objectionText, dealData, relevantContext) {
    try {
      ...
      // Format relevant context
      const contextSummaries = relevantContext && relevantContext.length > 0 ? relevantContext.map(ctx => {
        return `- ${ctx.type}: ${ctx.summary}`;
      }).join('\n') : 'No relevant context available';
  ...
  async generateWinLossAnalysisWithAI(dealData) {
    try {
      ...
      // Create a structured prompt for the AI
      const prompt = `
      You are an AI-powered Win-Loss Analyzer for a CRM system. Analyze this ${isWon ? 'won' : 'lost'} deal and explain the key factors.
