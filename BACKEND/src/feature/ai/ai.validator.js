...
const objectionHandlerSchema = Joi.object({
  objection_text: Joi.string().required().min(5).max(500).messages({
...
const embeddingGenerationSchema = Joi.object({
  ...
  limit: Joi.number().integer().min(1).max(500).optional().messages({
...
export const objectionHandlerValidator = validator(
  objectionHandlerSchema,
  "body"
);
export const embeddingGenerationValidator = validator(
  embeddingGenerationSchema,
  "body"
);
