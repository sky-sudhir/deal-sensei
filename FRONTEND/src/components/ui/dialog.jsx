...
const DialogContent = ({ className, children, onClose, ...props }) => (
  <>
    <DialogOverlay onClick={onClose} className="mt-0 pt-4" />
    <div
      className={cn(
        "fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-1 border bg-background p-4 pt-3 shadow-lg transition-all sm:rounded-lg",
        className
      )}
      {...props}
    >
...
const DialogHeader = ({ className, ...props }) => (
  <div
    className={cn(
      "flex flex-col space-y-0.5 text-center sm:text-left mb-0",
      className
    )}
    {...props}
  />
